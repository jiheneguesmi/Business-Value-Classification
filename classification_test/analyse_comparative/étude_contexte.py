...
CTX_TENSOR_PATH = r"F:\Jihene\business_value_classification\Business-Value-Knowledge-Graph\classification_test\Classification\resultats_classification_paragraphes\tensor_complet_20260520_114800.npy"
CTX_META_PATH   = r"F:\Jihene\business_value_classification\Business-Value-Knowledge-Graph\classification_test\Classification\resultats_classification_paragraphes\tensor_complet_20260520_114800_meta.json"
CTX_EXCEL_PATH  = r"F:\Jihene\business_value_classification\Business-Value-Knowledge-Graph\classification_test\Classification\resultats_classification_paragraphes\recapitulatif_complet_20260520_114800.xlsx"

# ── Version sans contexte (v5 phrases) ────────────────────────────────────────
NOC_TENSOR_PATH = r"F:\Jihene\business_value_classification\Business-Value-Knowledge-Graph\classification_test\Classification\resultats_classification_phrases\tensor_complet_20260517_074344.npy"
NOC_META_PATH   = r"F:\Jihene\business_value_classification\Business-Value-Knowledge-Graph\classification_test\Classification\resultats_classification_phrases\tensor_complet_20260517_074344_meta.json"
NOC_EXCEL_PATH  = r"F:\Jihene\business_value_classification\Business-Value-Knowledge-Graph\classification_test\Classification\resultats_classification_phrases\recapitulatif_complet_20260517_074344.xlsx"

# ── Dossier de sortie pour les résultats ──────────────────────────────────────
OUTPUT_DIR = r"F:\Jihene\business_value_classification\Business-Value-Knowledge-Graph\classification_test\analyse_comparative\résultats_analyse_comparative"

# ══════════════════════════════════════════════════════════════════════════════
#  CHARGEMENT DES DONNÉES
# ══════════════════════════════════════════════════════════════════════════════

def get_phrase_col(df):
    for col in ["Phrase_Text", "Phrase"]:
        if col in df.columns:
            return col
    raise ValueError(f"Aucune colonne de texte trouvée parmi : {df.columns.tolist()}")
...
def load_data():
    ...
    def _load_one(tensor_path, meta_path, excel_path, label):
        T = np.load(tensor_path, allow_pickle=True)
    ...
    # Clé de jointure : (source_file, texte_exact_de_la_phrase)
    col_ctx = get_phrase_col(df_ctx)
    col_noc = get_phrase_col(df_noc)

    df_ctx = df_ctx.dropna(subset=["Source_File", col_ctx]).copy()
    df_noc = df_noc.dropna(subset=["Source_File", col_noc]).copy()

    df_ctx["_key"] = df_ctx["Source_File"].astype(str) + "||" + df_ctx[col_ctx].astype(str)
    df_noc["_key"] = df_noc["Source_File"].astype(str) + "||" + df_noc[col_noc].astype(str)

    common_keys = sorted(set(df_ctx["_key"]) & set(df_noc["_key"]))
    ...
    df_ctx_common = df_ctx[df_ctx["_key"].isin(common_keys)].drop_duplicates(subset=["_key"], keep="first").set_index("_key")
    df_noc_common = df_noc[df_noc["_key"].isin(common_keys)].drop_duplicates(subset=["_key"], keep="first").set_index("_key")
...
def axe2_effet_contexte(ctx, noc, common_keys, vectors_ctx, vectors_noc):
    ...
    for qk in qkeys:
        ...
        n_oui_vers_non     = int(((v_ctx == 1) & (v_noc == 0)).sum())
        n_non_vers_oui     = int(((v_ctx == 0) & (v_noc == 1)).sum())
    ...
    top_flip_phrases = pd.DataFrame([
        {
            ...
            "Phrase_Text":  (df_ctx.loc[k, get_phrase_col(df_ctx)]
                             if k in df_ctx.index else "")[:120],
  ...
  │    → Le paragraphe "contamine" l'évaluation de la phrase.               │
  │                                                                          │
  │  • Direction "NOC plus permissif" (CTX=non, NOC=oui) :                  │
  │    → La phrase hors contexte est sur-interprétée.                        │
  │                                                                          │
  │  • Phrases avec 5+ flips → phrases très ambiguës nécessitant une        │
  │    annotation manuelle.                                                  │
  └─────────────────────────────────────────────────────────────────────────┘
    """)
...
def axe3_accord_intermodeles(ctx, noc):
    ...
    def _compute_intermodel_stats(tensor, meta, label):
        ...
        for qi, qk in enumerate(qkeys):
            slice_q   = tensor[:, :, :, qi]
            flat_votes= slice_q.reshape(-1, M)
            votes_oui = flat_votes.sum(axis=1)
  ...
  ├─────────────────────────────────────────────────────────────────────────┤
  │  • Delta_Accord positif (CTX > NOC) → avec contexte, les modèles        │
  │    s'accordent mieux. Le contexte réduit l'ambiguïté.                   │
  │  • Delta_Accord négatif (NOC > CTX) → le contexte introduit de la       │
  │    divergence entre modèles.                                             │
  │  • Entropie_CTX < Entropie_NOC → votes plus concentrés avec contexte.  │
  └─────────────────────────────────────────────────────────────────────────┘
    """)
...
# ══════════════════════════════════════════════════════════════════════════════
#  AXE 4 — ANALYSE PAR QUESTION (Q1–Q9)
# ══════════════════════════════════════════════════════════════════════════════
...
def axe4_par_question(ctx, noc, common_keys, vectors_ctx, vectors_noc, df_flips):
    ...
    sns.heatmap(norm_data, ax=axes[0], annot=True, fmt=".1f",
    ...
    print(f"\n  Questions classées par sensibilité :")
  ...
  ├─────────────────────────────────────────────────────────────────────────┤
  │  • Score_Sensibilite élevé = question fortement impactée par le contexte│
  │  • Questions rang 1–3 : questions critiques à analyser manuellement.    │
  │  • Accord_IM_CTX % < Accord_IM_NOC % → le contexte divise les modèles. │
  └─────────────────────────────────────────────────────────────────────────┘
    """)
...
def axe5_caracteristiques_phrases(ctx, noc, common_keys, vectors_ctx, vectors_noc):
    ...
    for key in common_keys:
        ...
        phrase_text = str(row_ctx.get(col_ctx, ""))
    ...
    axes[1,0].plot(x_line, p(x_line), "r--", lw=1.5, label="Tendance")
    ...
    sns.heatmap(pivot_lc, ax=axes[1,1], annot=True, fmt=".0f",
  ...
  ├─────────────────────────────────────────────────────────────────────────┤
  │  • Phrases courtes (≤8 mots) avec Pct_Flipped élevé → ambiguës sans    │
  │    contexte. La version CTX est plus fiable pour ce type.               │
  │  • Pct_Flipped plus élevé "Avec pronoms" → le contexte remplit bien     │
  │    son rôle de résolution d'anaphore.                                   │
  │  • Matrice de changement de label : cases hors-diagonale = changements  │
  │    de catégorie entre CTX et NOC.                                        │
  └─────────────────────────────────────────────────────────────────────────┘
    """)
...
def axe6_couts_scalabilite(ctx, noc):
    ...
    w = 0.35
    axes[2].bar(x - w/2, df_dist["CTX"],DSSSSSSSXS w, label="CTX", color="#2ecc71", alpha=0.85)
  ...
  ├─────────────────────────────────────────────────────────────────────────┤
  │  • Ratio_CTX_sur_NOC > 1.5 → surcoût fort, doit être justifié par un   │
  │    gain de qualité mesuré aux axes 1–4.                                 │
  │  • Distribution labels : CTX produit plus de "ROI"/"OBL" → le contexte │
  │    enrichit la détection. Plus de "Description" → prudence excessive.   │
  └─────────────────────────────────────────────────────────────────────────┘
    """)
...
def axe7_par_modele(ctx, noc, common_keys):
    ...
    def _model_vs_consensus(df, models, qkeys, label):
        rows = []
        # Extraire les vrais noms de modèles depuis les colonnes
        # Gère les deux formats: {model}_{qk} et {model}__{qk}
        actual_models = set()
        for col in df.columns:
            for qk in qkeys:
                # Essayer les deux formats: simple underscore et double underscore
                if col.endswith(f"_{qk}") and not col.startswith("AGG_") and not col.startswith("ACCORD_"):
                    model_name = col[:-len(qk)-1]  # Retirer _qk
                    actual_models.add(model_name.rstrip("_"))  # Normaliser (retirer underscores à la fin)
                elif col.endswith(f"__{qk}") and not col.startswith("AGG_") and not col.startswith("ACCORD_"):
                    model_name = col[:-len(qk)-2]  # Retirer __qk
                    actual_models.add(model_name.rstrip("_"))  # Normaliser
        
        for model in actual_models:
            for qk in qkeys:
                # Essayer simple underscore d'abord, puis double
                ind_col = f"{model}_{qk}"
                if ind_col not in df.columns:
                    ind_col = f"{model}__{qk}"  # Essayer format double underscore
        ...
        if not rows:
            return pd.DataFrame(columns=["Modele", "Question_Key", "Famille", "Version", "N", 
                                        "Accord_Cns_%", "Diverge_+_%", "Diverge_-_%", "Biais_Net"])
        return pd.DataFrame(rows)
    ...
    def _model_summary(df_mod):
        if len(df_mod) == 0:
            return pd.DataFrame(columns=["Modele", "Accord_Cns_Moy_pct", "Biais_Net_Moy", 
                                        "Diverge_Plus_Moy_pct", "Diverge_Moins_Moy_pct"])
    ...
    if len(sum_ctx) > 0 and len(sum_noc) > 0:
        ...
        df_comp_mod = pd.DataFrame(columns=["Modele", "Accord_CTX", "Biais_CTX", 
                                           "Accord_NOC", "Biais_NOC", "Delta_Accord", "Delta_Biais"])
    ...
    if len(df_comp_mod) > 0:
        ...
        axes[1].set_xticklabels(df_comp_mod["Modele"].tolist(),
                                rotation=30, ha="right", fontsize=9)
        axes[1].set_ylabel("Biais net moyen (%)")
  ...
  ├─────────────────────────────────────────────────────────────────────────┤
  │  • Delta_Accord positif → ce modèle bénéficie du contexte.              │
  │  • Biais_CTX positif élevé → modèle "contaminé" par le paragraphe.     │
  │  • Accord_CTX ≈ Accord_NOC → le modèle ignore le contexte.             │
  └─────────────────────────────────────────────────────────────────────────┘
    """)
